...
# --- Dataset class for RGB -> NIR mapping ---
class RGBNIRPairedDataset(Dataset):
    ...
    def _load_and_process_item(self, idx):
        ...
        try:
            ...
            print(f"Error loading or processing image {rgb_file}: {e}")
            # Return None or raise a custom exception to be handled by a collate_fn if desired
            # For now, returning None for all expected tensors, Dataloader should skip if collate_fn handles None
...
class DiscriminatorSmall(nn.Module):
    ...
    def forward(self, condition_rgb, target_nir):
        x = torch.cat([condition_rgb, target_nir], dim=1)
        return self.model(x)

# --- Gradient Penalty ---
def compute_gradient_penalty(critic, real_target_nir, fake_target_nir, condition_input_rgb, device):
    ...
    condition_input_rgb_matched = condition_input_rgb[:current_batch_size]
    interpolated_scores = critic(condition_input_rgb_matched, interpolated_nir)
...
def train_gan(config):
    ...
    for epoch in range(start_epoch, config['num_epochs']):
        ...
        if (epoch) % config.get('save_epoch_freq', 5) == 0 or (epoch) == config['num_epochs']:
...
if __name__ == "__main__":
    ...
    config = {
        ...
        'log_dir': 'runs/rgb_to_nir_wgan_gp_v2_3_256_from_checkpoint_v2', # New log_dir for 128px
        'checkpoint_dir': 'models/rgb_to_nir_wgan_gp_v2_3_256', # New checkpoint_dir for 128px
        'log_freq_batch': 100, 
        ...
        'checkpoint_load_epoch': 39, # Specify epoch to load if load_checkpoint is True
